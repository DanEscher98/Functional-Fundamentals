...
data Arbol a b = Hoja | Fruta b | Rama a (Arbol a b) (Arbol a b)
    deriving (Eq, Show)

sortListInTree :: Ord a => [a] -> Arbol a a
sortListInTree [] = Hoja
sortListInTree (x:xs) = loop xs (Fruta x) where
    loop [] ts     = ts
    loop (x:xs) ts = loop xs (insertInTree x ts)

insertInTree :: Ord a => a -> Arbol a a -> Arbol a a
...
inTree :: (Eq a, Ord a) => a -> Arbol a a -> Bool
inTree x Hoja         = False
